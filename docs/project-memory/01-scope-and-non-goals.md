...
| A third new technology **for the application itself** (e.g. TimescaleDB, Prometheus/Grafana, a message queue) | Rule D3 froze this repo's learning budget at exactly two new technologies (Go concurrency, OTel pipelines) in Session 0 (`00a-ledger-confirmation.md`); the retention/rollup decision in `00-project-brief.md` §3 already reasons through why plain Postgres is chosen instead of TimescaleDB specifically. **Superseded in part, for the deployment layer only, by ADR-0005 (Session 14):** the project owner explicitly confirmed a supplementary budget of two more technologies — Terraform and Kubernetes — to give this repo's own Release & Deployment / Operations & Maintenance deep phases a real second deployment target, since `docker compose` alone cannot demonstrate a genuine rolling upgrade or a declarative, rebuildable infrastructure story. This is a reasoned, on-the-record amendment (see ADR-0005 and `09-decision-log.md`'s Session 14 entry), not a silent violation — and it does not reopen this row's own original scope: TimescaleDB, Prometheus/Grafana, and a message queue remain excluded from the *application*, for the same reasons as before. | Application-layer technologies: only against a future session's *measured* data volume showing the hand-rolled rollup job can't keep up. Deployment-layer technologies beyond Terraform/Kubernetes: would need their own fresh ADR, not an extension of this one. |
