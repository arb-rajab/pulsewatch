...
|---|---|---|---|---|
| B-009 | Real-cluster verification of `infra/terraform/` + `deploy/k8s/` (a real `terraform init`/`plan`/`apply` and a real `deploy/k8s/rollout.sh` reaching `Running` pods) | Infra / Verification | Medium | ADR-0005 (Session 14) authored the Terraform module and Kubernetes manifests but this sandbox had no registry egress or live cluster/Docker daemon to apply against (R-008, `10-risk-register.md`) — this project's own standard is real evidence, not "looks right," and this is the one part of Session 14 that could only be offline-validated |
