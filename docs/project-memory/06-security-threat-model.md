...
| T-11 | Browser ↔ `GET /api/v1/events` (ADR-0010) | An authenticated operator session (legitimate or stolen) opens unbounded concurrent SSE streams, each holding a goroutine, a heartbeat ticker, and a `livefeed.Hub` subscriber slot indefinitely | Denial of service | Low / Low | **Closed this session.** `sseConnLimiter` (`backend/internal/operatorapi/sselimiter.go`) caps concurrent streams at 4 per operator and 50 total across all operators, in-process (no Redis — same "not load-bearing" reasoning as T-01's login limiter); a stream beyond either cap is refused with `429` before it subscribes to the hub. | `operatorapi/events_test.go` (`TestStreamEvents_CapsConcurrentConnectionsPerOperator`) |
