...
| Device push token (FCM/APNs) | `device_tokens.token` | Sensitive, **currently unencrypted** | Stored in plaintext (`backend/internal/alerting/devicetokens.go`) — a real, named gap, not an oversight; see "Accepted risks". |
...
| T-08 | Backend ↔ outbound webhook destination | Operator (or an attacker who has stolen the operator's session) configures a webhook destination pointing at an internal/private address (SSRF) | Elevation of privilege | Low / Medium | **No validation exists** — `CreateAlertChannel` accepts any syntactically valid destination string for a webhook channel; nothing resolves or blocklists private/loopback/link-local ranges before `WebhookDispatcher` (ADR-0006) makes the real HTTP request. Accepted for the single-trusted-operator model (the operator configuring their own channel is not crossing a privilege boundary in the way a multi-tenant SaaS would); would need revisiting if this system ever accepted alert-channel configuration from a less-trusted principal. | Gap — first documented this session, not previously named in ADR-0006 or the risk register |
| T-09 | Backend ↔ alert_channels / device_tokens (data at rest) | Database compromise exposes alert destinations / push tokens in plaintext | Information disclosure | Low / Medium | Webhook URLs, email addresses, and push credentials are AES-256-GCM encrypted at rest (`alerting/channel.go`), key from `ALERT_CHANNEL_ENCRYPTION_KEY` (not itself in the database). Device push tokens (`device_tokens.token`) are the one exception — stored in plaintext, no `EncryptDestination` call anywhere in `devicetokens.go`. | `alerting/channel_test.go` (encryption round-trip); no equivalent test for device tokens because none is encrypted |
...
|---|---|---|
| Device push tokens stored in plaintext (`device_tokens.token`, T-09) | Not yet given the same `EncryptDestination` treatment as `alert_channels.destination_encrypted` — an oversight relative to that precedent, not a deliberate design choice, but low severity today: a push token alone (without this project's own FCM/APNs service-account credentials, which live in K8s `Secret`s/env vars, not the database) cannot be used to send a push notification from outside this system. | Before B-016 (real FCM/APNs delivery with live credentials) or any session that treats `device_tokens` as equally sensitive to `alert_channels` — encrypt it the same way. |
...
| No SSRF guard on webhook destinations (T-08) | Single-trusted-operator model: the one operator configuring their own webhook destination is not crossing a privilege boundary the way a multi-tenant SaaS accepting webhook config from untrusted tenants would. | If multi-user/multi-tenant operator access is ever added (currently an explicit non-goal, `01-scope-and-non-goals.md`) — SSRF protection on any outbound-URL-accepting field becomes load-bearing at that point, not optional. |
